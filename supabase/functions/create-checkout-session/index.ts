...
// 2. Switched to Deno.serve to fix the "runMicrotasks" error
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. Optimized Stripe Client for Deno Edge Runtime
    ...
    // 4. Optimized Database Fetch with Single Query
    ...
    // 6. Stripe Session with metadata for UUID sync
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `Manolo AI Digital Pass for ${venue.name}`,
            },
      ...
      metadata: {
        user_id: user.id,
        venue_id: venue_id, // Sent as UUID
