...
## Profiles rows are never created automatically — systemic, not a one-off

Confirmed live (2026-08-03): **zero triggers on `auth.users`**, **no function anywhere inserts into `profiles`** (only `start_conversation` even references it, read-only), and `Auth.tsx`'s signup path calls `supabase.auth.signUp()` and nothing else. Live counts: **5 `auth.users`, 4 `profiles`**. So every new signup lands with no profile row, and every role-based code path treats that user as a profile-less guest. The CEO account missing its row is a symptom of this, not a special case. The four existing rows were created by hand or by tooling since removed. Needs a real `handle_new_user` trigger (or equivalent in the signup path) before onboarding anyone; patching individual rows by hand does not fix the mechanism.
