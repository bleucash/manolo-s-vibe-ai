...
Found live, mid-testing item 3: a `mode === "manager"` user owning no venue yet (i.e. between submitting a Tier 1 claim and approval) bounced forever between `/profile` and `/venue/manage`, `toast.error` firing every cycle. `Profile.tsx` sends every manager-mode user to `/venue/manage`; `VenueManage.tsx` sent them straight back the instant `activeVenueId` was falsy. Pre-existing, not introduced by the item 1-3 commits (verified via `git log`), it had just never been reachable before a manager account worked end to end.

Fixed in `VenueManage.tsx` only, `Profile.tsx` untouched: that branch no longer navigates at all, it renders one of two stable screens instead. It self-scopes a query for the user's most recent pending claim (nothing reusable existed, `useVenueStatus.hasPendingClaim` is per-venue and there's no venue in context at this point) and shows either "claim under review" naming the venue, or "no sector claimed" with a route into Discovery. Killing the redirect on this side alone breaks the cycle since `Profile.tsx` has nothing left to ping-pong against.

## Profiles auto-creation — closed (2026-08-03)

Every signup used to land profile-less: zero triggers on `auth.users`, nothing anywhere inserted into `profiles`, and `Auth.tsx` calls `signUp()` and nothing else (counts were 5 `auth.users` vs 4 `profiles`). The CEO account's missing row was a symptom, not a special case. Fixed by `handle_new_user()` + `on_auth_user_created` (`AFTER INSERT ON auth.users`), migration `20260803_handle_new_user_trigger.sql`, commit `4e13ba4`. Verified end to end with a real UI signup, not just by checking the trigger exists: the row was created automatically with `role_type: guest` and schema defaults, then the test rows were deleted, which also confirmed `profiles_id_fkey ON DELETE CASCADE` works.

**It inserts only `id`, deliberately, and that is not laziness.** `Auth.tsx` sends no `options.data`, so `raw_user_meta_data` holds only `{sub, email, email_verified, phone_verified}` on real rows: no username, no name, nothing but the email. And deriving anything from the email is actively unsafe here: `profiles` has `UNIQUE (username)`, so an email-derived username collides for `info@a.com` vs `info@b.com`, and a unique violation inside an AFTER INSERT trigger on `auth.users` **aborts the transaction and blocks signup entirely**. Separately, `profiles` SELECT is `USING (true)`, so an email-derived username would publish part of every user's email address. `role_type` falls to its `'guest'` default. **If richer defaults are wanted, the fix is `Auth.tsx` passing `options.data` at signup, not guessing from the email in SQL.**

`ON CONFLICT (id) DO NOTHING` keeps it idempotent, otherwise a signup for an already-patched id (like the CEO's) raises a PK violation and blocks that signup. No exception handler wraps the body on purpose: silently swallowing is how the profile-less users accumulated unnoticed, so this fails loudly instead.
...
**`venue_claims` RLS - closed (2026-08-03).** It had no row security at all (`relrowsecurity` was `false`, zero policies): every authenticated user could read every claim including Tier 2 PII, and could self-approve by flipping `status`. Now: RLS enabled, SELECT scoped to `user_id = auth.uid() OR is_admin()`, INSERT scoped to `user_id = auth.uid() AND status = 'pending'` (the status predicate is load-bearing, without it a claimant can insert a row pre-set to `approved`). **No UPDATE or DELETE policy exists deliberately** - approve/reject runs through `admin-actions` on the service role key, which bypasses RLS, so with no matching policy every client-side write is denied by default. Migration `20260803_venue_claims_rls.sql`, verified after running via `pg_policies` + `relrowsecurity`.

**New `public.is_admin()` helper, and it is a third copy of the admin identity.** RLS policies can't see the `ADMIN_USER_ID` secret and admin deliberately isn't in `role_type`, so the only in-DB admin signal is the JWT email claim, same mechanism as `CEORoute`. `is_admin()` exists so that email appears exactly once in the database rather than inline per policy. This widens the already-accepted email/secret desync risk above from two copies to three. Deliberate, logged, not an oversight.
