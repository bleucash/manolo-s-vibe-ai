...
**The request queue is BUILT (2026-08-31) and the hard reject is gone.** The rope is now a state, not a refusal: `conversation_participants.state` is `accepted` / `pending` / `declined`, and a guest messaging someone who does not follow them back creates the thread with the **recipient's** row pending. Talent and managers stay exempt; the exemption is a property of the sender's role and now picks the recipient's initial state rather than whether the call succeeds. **Not called `active`** — that word already means three things here, and a fourth was the trap this file warns about.

**Two helpers, split by read vs write.** `is_conversation_participant` is membership at any state and guards **reads**, because a pending recipient must see a request to judge it; `is_accepted_conversation_participant` guards **writes** (message INSERT, mark-read UPDATE). Without the split a pending recipient replies straight past the gate. **The state machine is a policy, not a function:** `USING` sees OLD and `WITH CHECK` sees NEW, so `USING (user_id = auth.uid() AND state = 'pending')` + `WITH CHECK (... IN ('accepted','declined'))` says you may only leave pending, only into those two, only on your own row, with no path back **into** pending. Paired with `GRANT UPDATE (state)` after revoking the broad grant, same mechanism as `messages.is_read`. Requests render **above and outside** the PRIMARY/GENERAL tabs, so fixing that split later touches none of it.

**Decline is a deliberate black hole.** The sender is not told and can keep writing into a thread the recipient will never see; blocking their inserts would leak the decline through an error. **Recovery is asymmetric on purpose:** `start_conversation` sets the **caller's own** row to accepted before its idempotent early return, so only the decliner can undo their own decline, and only by initiating. A sender can never re-request past it. **Unbounded requests are a pre-launch item** (see Build order).
