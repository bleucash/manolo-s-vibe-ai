...
**`venue_claims` has no RLS at all (found 2026-08-03, unfixed, treat as urgent):** confirmed directly, `pg_policies` returns zero rows for it and `pg_class.relrowsecurity` is `false`. Not "permissive policies", no row security whatsoever. Every authenticated user can read every claim, including `legal_name`, `business_email`, `business_phone` (Tier 2 PII), and can insert/update/delete arbitrary rows, e.g. self-approving a claim by flipping `status`, or deleting someone else's. Nothing in the app depends on this being open; `ClaimSectorModal` inserts only the caller's own row and `CEODashboard` reads as admin. Needs owner sign-off on the policy SQL like any other schema change.
