...
**An RLS-filtered write does not error. It returns 200 with zero rows.** Any call that only checks `if (error)` will report success while nothing happened, and the user stops looking. This has bitten six times: the manager approve path (said "Neural Link Confirmed" on an unverified venue), Build 2's self-approval test, Build 3's cross-user delete, `TalentDashboard.handleResponse` on a stale invite, Build 4's accept-twice case, and `markAsRead`, which had **no UPDATE policy at all** and so never marked a single message read for the life of the feature while the badge cleared optimistically and returned on every reload (fixed `20260830140000`). **Discipline: every write that depends on a policy ends in `.select()` and checks rows affected**, not just absence of error. Report accurately when the count is zero. A trigger that `RAISE`s is the exception, it surfaces as a real error; RLS never does.
...
**Five invented references found so far**, each written at a call site and each invisible until something checked it: `conversation_summary.unread_count` (never existed, `|| 0` made the badge permanently dark), `posts.updated_at`, `conversations.user1_id`/`user2_id`, `public.follows` (2026-08-30), and `/messages?conversation=<id>` — a URL contract two callers built and `Messages.tsx` never read, so the thread landed unselected (wired 2026-08-30). **`public.follows` is the one to remember: it lived in a SQL function body, where wiring TypeScript types was never going to catch it.** The param is the second lesson: **types only check what crosses a typed boundary. A URL, a query string, or a `postgres_changes` payload is a contract with nothing enforcing it.** Postgres does not validate a `plpgsql` body against the schema at `CREATE` time — a function referencing a nonexistent table compiles, deploys, and fails only when that branch first executes. `start_conversation` sat broken for every guest and manager caller from the day it shipped. **SQL function bodies need the same verification discipline as migrations: check every table and column named in them against the live schema, and execute every branch.** A green deploy proves nothing.
...
**Naming collision, unresolved:** `Discovery.tsx` defines its own local `FollowButton`, a different component from `@/components/FollowButton`. Worth renaming when that file is next opened. **Existing data oddity, leave it alone:** `followers` holds a self-follow row (`Money Machine` follows itself); `useFollow` guards against it client-side (`currentUserId === targetId`), so it predates the guard or arrived another way. Noted rather than cleaned, since deleting real rows mid-verification is how fixtures get confused with production data.
...
**`events.created_by` references `auth.users`, not `profiles`** — same shape as `venues.owner_id`. PostgREST cannot embed `profiles` through it, so any future query must fetch creators in a second call keyed on `created_by`, as `CEODashboard` does for venue owners. **Second PostgREST trap, confirmed against live 2026-08-30: `.or()` cannot reference an embedded table's column.** `.select("user_id, venues(owner_id)").or("user_id.eq.X,venues.owner_id.eq.X")` returns **400 `PGRST100` "failed to parse logic tree"** — the whole request fails, no rows. `or` is parsed against the top-level table only; a dotted path inside it is not a column. The same query with a plain top-level filter returns 200, so the embed is not the problem, the `or` is. **Do instead:** two queries unioned client-side, or a view/RPC that expresses the disjunction in SQL. This is doubly dangerous with `const { data } = await …` and no `error`: the 400 is swallowed and the branch silently contributes nothing, which is exactly how `ContactsList`'s professional-contacts half never returned a row.
...
## Messaging — built 2026-08-31

**Intended design:** talent and managers message freely into anyone's inbox; guests can message anyone but land in a request queue unless the recipient follows them back (Instagram's model; the filter lives on the receiving end, since a follow-gate is trivially bypassed by following first). **Refinement (owner, 2026-08-07):** managers are exempt from that friction, businesses don't want it turning away customers. **Cosmetic, observed 2026-08-30, deliberately unfixed.** (1) A thread lands in a different tab depending on who looks: `Messages.tsx` puts it in Primary if `display_name?.toLowerCase().includes("manager")`, so the same thread is Primary for the guest and General for the manager, decided by a name string. (2) Uppercase leaks onto **user content** in two places — the composer input (`ChatWindow.tsx:132`) and the sidebar snippet (`Messages.tsx:90`). The all-caps look is a deliberate app-wide idiom, not an inherited class, and the bubble body itself was correctly left alone, so this is the idiom misapplied rather than a styling accident. The composer is the worse of the two: it displays what you type in caps while storing normal case. (3) Two different fallbacks for one NULL `display_name`: "NEURAL USER" in the sidebar, "NEURAL NODE" in the `ChatWindow` header.

**The request queue is BUILT (2026-08-31) and the hard reject is gone.** The rope is now a state, not a refusal: `conversation_participants.state` is `accepted` / `pending` / `declined`, and a guest messaging someone who does not follow them back creates the thread with the **recipient's** row pending. Talent and managers stay exempt; the exemption is a property of the sender's role and now picks the recipient's initial state rather than whether the call succeeds. **Not called `active`** — that word already means three things here, and a fourth was the trap this file warns about. **Two helpers, split by read vs write.** `is_conversation_participant` is membership at any state and guards **reads**, because a pending recipient must see a request to judge it; `is_accepted_conversation_participant` guards **writes** (message INSERT, mark-read UPDATE). Without the split a pending recipient replies straight past the gate. **The state machine is a policy, not a function:** `USING` sees OLD and `WITH CHECK` sees NEW, so `USING (user_id = auth.uid() AND state = 'pending')` + `WITH CHECK (... IN ('accepted','declined'))` says you may only leave pending, only into those two, only on your own row, with no path back **into** pending. Paired with `GRANT UPDATE (state)` after revoking the broad grant, same mechanism as `messages.is_read`. Requests render **above and outside** the PRIMARY/GENERAL tabs, so fixing that split later touches none of it. **Decline is a deliberate black hole.** The sender is not told and can keep writing into a thread the recipient will never see; blocking their inserts would leak the decline through an error. **Recovery is asymmetric on purpose:** `start_conversation` sets the **caller's own** row to accepted before its idempotent early return, so only the decliner can undo their own decline, and only by initiating. A sender can never re-request past it. **Unbounded requests are a pre-launch item** (see Build order). **`start_conversation(target_user_id)` is the only correct way to open a thread** (repaired `20260830120000`, all branches executed against a fixture). It is idempotent — an existing two-person thread is returned rather than a second one created — and takes a transaction-scoped advisory lock on the ordered pair, so a double-pressed button cannot race two threads into existence. It returns an existing thread **before** applying the velvet rope, so withdrawing a follow does not lock someone out of a conversation both people can already see. Do not hand-roll insert-then-participants: that is two writes with a window that leaves a conversation with no participants, invisible to every query but still a row.

**Group chat is intended, not hypothetical** — primarily venue-to-staff coordination. `conversation_summary` therefore returns **NULL `display_name` and `avatar_url` whenever there is more than one other participant**, so the client can branch on it. Picking a participant deterministically was rejected: it renders one confidently wrong name on a three-way thread, the same failure shape as the inverted badge. NULL is honest, and this is the foundation rather than a placeholder. **Design pass still open:** thread naming, who may add and remove participants, whether venue threads follow `venue_staff` affiliation so leaving the venue removes you, and how roles surface in a multi-party thread. **Deferred idea worth keeping** (from the deleted `ContactsList`): a "who can I message" list unioning *social* contacts (people you follow) with *professional* ones (your manager if you are talent, your staff if you are a manager, via active `venue_staff`). The union is the useful part — a talent's colleagues are not necessarily people they follow. Nothing implements it; the deleted file's attempt never worked.
...
Items 1-6 closed 2026-08-02 to 2026-08-11: phantom purge, claim modal fix, `Dashboard` wiring, talent onboarding, `posts` INSERT lock, Tier 2 verification. Builds 1-4 closed 2026-08-17 to 2026-08-21: position enum + config, talent-requests-venue, check-in constraint + talent exits, manager-invites-talent. **Item 8, the messaging follow-gate, closed 2026-08-31** (request queue; see Messaging).
...
**Pre-launch, before signups open:** bound the number of pending message requests a guest can hold. Unlimited is fine at hand-gatekept scale and is the first thing abused when it is not.
