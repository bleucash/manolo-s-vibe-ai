...
- **Tier 2 UI (a)-(d) all built 2026-08-11, NOT YET ROUND-TRIP TESTED.** (a) `business_verified` rides along on `UserModeContext`'s existing `userVenues` select (zero new queries), read via `useVenueVerified(venueId)`; a querying hook was rejected because all five gated surfaces mount at once for the same venue and would fire the identical lookup five times per dashboard. (b) All five render a shared `Tier2Notice` and disable their controls when unverified, replacing "fully enabled, then fail at write time"; visually confirmed. (c) `BusinessVerificationModal` (four fields, per-field validation, `23505` treated as "already under review" since one-pending-per-venue makes that routine, not a race) is owned by `Tier2Notice`, so all five surfaces got the trigger from one edit. (d) Third `CEODashboard` tab mirroring the talent queue against `approve_business`/`reject_business`. **The untested part is the loop: file → CEO queue → approve → `business_verified` flips → all five unblock.** That is exactly the shape that hid the `ADMIN_USER_ID` 403 for weeks, a payload or RLS mismatch invisible until the whole path runs. Do not treat build order item 6 as closed until it passes. Also fixed here: `VenuePriceEditor` was surfacing `err.message` straight to the user, i.e. the raw `venues_require_business_verified` trigger exception. **Approve/reject asymmetry closed (migration `20260811_venue_staff_delete_tier2_gate.sql`):** the Tier 2 pass gated `venue_staff` UPDATE but not DELETE, so on an unverified venue approve was refused by the DB while reject succeeded. Both predicates are now identical. UI disabling is not a substitute for the policy, RLS is the boundary. Known rough edge: `Tier2Notice`'s "under review" state is local, so a reload makes the prompt look actionable again until the `23505` path corrects it.
...
6. **Enforcement closed (2026-08-09), UI built but NOT round-trip tested (2026-08-11), so item 6 is not closed.** Tier 2 business verification, see Tier 2 above. Gated via trigger on `venues` (column granularity, RLS is row-level and would have broken Tier 1 hero reel) and via policy on `venue_staff`/`payout_history`. Migrations `20260809_tier2_business_verification.sql`, `20260811_venue_staff_delete_tier2_gate.sql`. Filing modal and CEO review tab exist; **close this only once file → review → approve → unblock has actually been run end to end.**
