...
- **Tier 2 UI (a)-(d) built and round-trip verified 2026-08-11.** (a) `business_verified` rides along on `UserModeContext`'s existing `userVenues` select (zero new queries), read via `useVenueVerified(venueId)`; a querying hook was rejected because all five gated surfaces mount at once for the same venue and would fire the identical lookup five times per dashboard. (b) All five render a shared `Tier2Notice` and disable their controls when unverified, replacing "fully enabled, then fail at write time". (c) `BusinessVerificationModal` (four fields, per-field validation, `23505` treated as "already under review" since one-pending-per-venue makes that routine, not a race) is owned by `Tier2Notice`, so all five surfaces got the trigger from one edit. (d) Third `CEODashboard` tab mirroring the talent queue against `approve_business`/`reject_business`, which take `{ venue_id }` not `{ user_id }`. **Full loop run on real accounts against production: manager filed all four fields, notice flipped to under review, application surfaced in the CEO tab with fields intact, approved there, all five surfaces unblocked. Confirmed in the DB afterward, `venues.business_verified = true` and application `status = 'approved'`, so both writes in `approve_business` landed and the partial-failure mode did not occur.** Also fixed here: `VenuePriceEditor` was surfacing `err.message` straight to the user, i.e. the raw `venues_require_business_verified` trigger exception. **Approve/reject asymmetry closed (migration `20260811_venue_staff_delete_tier2_gate.sql`):** the Tier 2 pass gated `venue_staff` UPDATE but not DELETE, so on an unverified venue approve was refused by the DB while reject succeeded. Both predicates are now identical. UI disabling is not a substitute for the policy, RLS is the boundary. Known rough edge: `Tier2Notice`'s "under review" state is local, so a reload makes the prompt look actionable again until the `23505` path corrects it.
- **Tier 2 (business verification) — built and enforced (2026-08-09, migration `20260809_tier2_business_verification.sql`).** The four business columns on `venue_claims` were vestigial: nullable, NULL on every live row, and read/written by nothing. **The earlier note here saying this "needs enforcement, not a schema change" was wrong** and is corrected: there was no Tier 2 state to enforce against, so state had to be built first. Now `venues.business_verified boolean NOT NULL DEFAULT false` is the flag, and `venue_business_applications` (venue_id + user_id NOT NULL, all four business fields NOT NULL, partial unique index one pending per venue, RLS mirroring `talent_applications` plus a venue-ownership requirement on INSERT) is the request queue. `admin-actions`' `approve_business`/`reject_business` set the flag first then close the application, same ordering as `approve_talent`. The filing surface and admin queue that were outstanding here are now built, see the Tier 2 UI entry above.
...
6. **Closed (2026-08-11, verified end to end).** Tier 2 business verification, state, enforcement and UI, see Tier 2 above. Gated via trigger on `venues` (column granularity, RLS is row-level and would have broken Tier 1 hero reel) and via policy on `venue_staff`/`payout_history`. Migrations `20260809_tier2_business_verification.sql`, `20260811_venue_staff_delete_tier2_gate.sql`. Commits `74cbb77` (a+b), `0eb0557` (c+d). File → CEO review → approve → all five surfaces unblock was run on real manager and CEO accounts and passed, with `business_verified` and application `status` both confirmed in the DB after.
