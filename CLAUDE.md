...
**OPEN, confirmed live 2026-08-22: `/messages` returns every conversation in the database to any authenticated user.** Two causes compounding. `conversation_summary` has no `security_invoker` option, so it keeps Postgres's default and runs with the owner's rights, **bypassing RLS on `messages`** (which is otherwise enabled and policied). And `useChat.ts:55` filters only `.neq("participant_id", currentUserId)`, i.e. "not me", with no scoping to the caller's own conversations. Demonstrated with a fixture: a conversation between two throwaway users, with the signed-in account not a participant, came back over the API with `last_message_content` fully readable. The UI happened not to paint those rows, but the message bodies reached the browser. Unexposed today only because `conversations` and `messages` hold zero rows. **Fixing it means designing the query and the `conversation_participants` policy together**, including how the other participant's name and avatar survive under invoker rights.

**Same root cause, second bug: the unread badge is inverted.** Because the client reads the *other* participant's row, `unread_count` on that row is how many messages **they** have not read. Verified: with the caller's own unread at 0 and the counterparty's at 1, the badge still shows. It lights up when someone hasn't read *your* message. Fix belongs with the query redesign above, not separately.
